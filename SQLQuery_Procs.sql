...
BEGIN
	...
	BEGIN
		...
		DECLARE @ID_ALUNO_NOVO INT;
		SET @ID_ALUNO_NOVO = (SELECT SCOPE_IDENTITY());
		RETURN @ID_ALUNO_NOVO
	END
	ELSE 
	BEGIN
...
BEGIN
	...
	SET
		FLAG_ATIVO = 0
	WHERE 
		ID_TREINO = (SELECT TOP 1 ID_TREINO FROM TB_TREINO WHERE ALUNO_TREINO = @ALUNO_TREINO AND FLAG_ATIVO = 1 ORDER BY DATA_INCLUSAO_TREINO DESC)
		AND ALUNO_TREINO = @ALUNO_TREINO
		AND FLAG_ATIVO = 1
...
CREATE PROCEDURE SP_GRAVA_TREINO_DIA
	...
	@FICHA_TREINO_DIA VARCHAR(10),
	@GRUPO_MUSCULAR_TREINO_DIA VARCHAR(255)
AS
BEGIN
	INSERT INTO TB_TREINO_DIA (TREINO_TREINO_DIA, FICHA_TREINO_DIA, GRUPO_MUSCULAR_TREINO_DIA)
	VALUES
	(
		@TREINO_TREINO_DIA,
		@FICHA_TREINO_DIA,
...
END
EXEC SP_ESPECIFICA_TREINO_DIA 17, 'PEITO'
...
CREATE PROCEDURE SP_REMOVE_TREINO_FICHA
	@ID_EXERCICIO_TREINO INT
AS
BEGIN
	DELETE TB_EXERCICIO_TREINO 
	WHERE ID_EXERCICIO_TREINO = @ID_EXERCICIO_TREINO
END

GO

CREATE PROCEDURE SP_CANCELA_MONTAGEM_TREINO
	@ID_TREINO INT
AS
BEGIN
	DELETE FROM TB_EXERCICIO_TREINO WHERE TREINO_EXERCICIO_TREINO = @ID_TREINO
	DELETE FROM TB_TREINO_DIA WHERE TREINO_TREINO_DIA = @ID_TREINO
	DELETE FROM TB_TREINO WHERE ID_TREINO = @ID_TREINO
END
...
BEGIN
	...
	WHERE ALUNO_TREINO = @ID_ALUNO AND TRE.FLAG_ATIVO = 1
	ORDER BY DATA_INCLUSAO_TREINO DESC

	SELECT TRE.ID_TREINO, TRE.ALUNO_TREINO, TRE.NR_TREINO_ALUNO, TRE.MODELO_TREINO, TRE.FOCO_TREINO, DPA.NOME_ALUNO, TRD.ID_TREINO_DIA, TRD.FICHA_TREINO_DIA, TRD.GRUPO_MUSCULAR_TREINO_DIA
	FROM TB_TREINO AS TRE
	INNER JOIN TB_TREINO_DIA AS TRD ON TRD.TREINO_TREINO_DIA = TRE.ID_TREINO
	INNER JOIN TB_DADOS_PESSOAIS_ALUNO AS DPA ON DPA.ID_ALUNO = TRE.ALUNO_TREINO
	WHERE ID_TREINO = @ID_TREINO AND TRE.FLAG_ATIVO = 1
	ORDER BY DATA_INCLUSAO_TREINO DESC
...
END
--GO
EXEC SP_CONSULTA_TREINO_ATUAL 2
GO
